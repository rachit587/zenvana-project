...
const Layout = ({ children, userId, currentPage }) => {
    const auth = getAuth();
        ...
        <div className="min-h-screen flex bg-gradient-to-br from-gray-950 to-gray-900 font-sans text-gray-100">
            <nav className="w-64 bg-gray-900 shadow-lg p-6 flex flex-col rounded-r-3xl transition-all duration-300 ease-in-out transform hover:shadow-2xl">
                <div className="mb-10 text-center">
                    ...
                    {userId && (
                        <p className="text-xs text-gray-400 mt-2">User ID: <span className="font-mono bg-gray-800 px-2 py-1 rounded-md text-gray-300 break-all">{userId}</span></p>
                    )}
                </div>
                <ul className="space-y-4 flex-grow">
                    {navigationItems.map((item) => (
                        <li key={item.name}>
                            <Link to={item.path} className={`w-full text-left flex items-center p-3 rounded-xl transition-all duration-200 ease-in-out transform hover:scale-105 hover:bg-gray-700 hover:text-green-400 ${currentPage === item.name.toLowerCase().replace(' ', '-') ? 'bg-gray-700 text-green-400 shadow-md' : 'text-gray-300 hover:text-gray-100'}`}>
                                {item.icon}
                                {item.name}
                            </Link>
                        </li>
