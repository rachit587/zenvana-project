...
function App() {
  ...
  const saveFinancialData = async (data) => {
    ...
    try {
      ...
      setFinancialSummary(dataToSave); // This will trigger the useEffect below
    } catch (error) {
      console.error("!!! Critical Error saving data:", error);
      setIsSubmitting(false); // Ensure button is re-enabled on error
      throw error;
    } 
    // The finally block is removed to let the useEffect handle the state change
  };
  ...
  useEffect(() => {
    ...
    if (financialSummary && currentPage === 'onboarding') {
      ...
      setIsSubmitting(false); // Reset submitting state after successful navigation
    }
