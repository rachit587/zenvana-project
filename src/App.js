...
/** Reads env safely and trims whitespace */
const env = (key) => (process.env[key] || '').trim();

/* ---------- CONFIG: Firebase (from Netlify env) ---------- */
...
/* ---------- CONFIG: AI keys (Groq for now) ---------- */
/** CRA normally exposes only REACT_APP_*; we also fall back to plain GROQ_API_KEY so your current Netlify works */
const GROQ_API_KEY = env('REACT_APP_GROQ_API_KEY') || env('GROQ_API_KEY');
  ...
  return (
    <div className="p-4 bg-red-900/70 text-red-100 border border-red-700 rounded-xl m-4">
      ...
      <ul className="list-disc ml-6 text-sm">
        {items.map((i) => <li key={i}><code>{i}</code> is not set in environment variables.</li>)}
      </ul>
      <p className="text-xs mt-2 opacity-80">
        Add these in Netlify (Site settings → Environment variables) and redeploy.
      </p>
    </div>
  );
};
      ...
      <div className="mb-10 text-center">
        ...
        {userId && (
          <p className="text-xs text-gray-400 mt-2">
            User ID:{' '}
            <span className="font-mono bg-gray-800 px-2 py-1 rounded-md text-gray-300 break-all">
              {userId}
            </span>
          </p>
        )}
      ...
      <ul className="space-y-4 flex-grow">
        <li>
          <button
            ...
            className={`w-full text-left flex items-center p-3 rounded-xl transition-all duration-200 ease-in-out transform hover:scale-105 hover:bg-gray-700 hover:text-green-400 ${currentPage === 'dashboard' ? 'bg-gray-700 text-green-400 shadow-md' : 'text-gray-300 hover:text-gray-100'}`}>
        ...
        <li>
          <button
            ...
            className={`w-full text-left flex items-center p-3 rounded-xl transition-all duration-200 ease-in-out transform hover:scale-105 hover:bg-gray-700 hover:text-green-400 ${currentPage === 'taxSaver' ? 'bg-gray-700 text-green-400 shadow-md' : 'text-gray-300 hover:text-gray-100'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1l-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
            Tax Saver
          </button>
        </li>
        <li>
          <button
            ...
            className={`w-full text-left flex items-center p-3 rounded-xl transition-all duration-200 ease-in-out transform hover:scale-105 hover:bg-gray-700 hover:text-green-400 ${currentPage === 'aiChat' ? 'bg-gray-700 text-green-400 shadow-md' : 'text-gray-300 hover:text-gray-100'}`}>
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-950 to-gray-900 text-gray-100 relative">
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-6xl mx-auto px-4 py-20 text-center">
        <h1 className="text-7xl md:text-8xl font-extrabold text-white mb-4 drop-shadow-lg bg-clip-text text-transparent bg-gradient-to-r from-green-300 to-yellow-300">
          Welcome to ZENVANA
        </h1>
        <p className="text-2xl md:text-3xl text-gray-300 mb-10 max-w-3xl">
          Your AI financial advisor for your financial freedom
        </p>
        ...
        <section className="bg-gray-800 bg-opacity-70 backdrop-blur-sm p-8 rounded-3xl shadow-2xl border border-gray-700 mb-8 max-w-3xl w-full transition-all duration-300 hover:shadow-green-glow">
          ...
          <p className="text-xl text-gray-300 leading-relaxed">
            We believe financial expertise shouldn't be a luxury. Our mission is to empower every Indian with a personal AI advisor, making financial well-being and peace of mind a reality for all.
          </p>
        </section>

        <button
          onClick={onGetStarted}
          className="bg-gradient-to-r from-green-600 to-yellow-600 hover:from-green-700 hover:to-yellow-700 text-gray-900 font-bold py-5 px-12 rounded-full text-2xl transition duration-300 ease-in-out transform hover:-translate-y-2 hover:shadow-gold-glow shadow-lg mb-20">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .shadow-gold-glow{box-shadow:0 0 25px rgba(255,215,0,.6)}
        .shadow-green-glow{box-shadow:0 0 20px rgba(16,185,129,.5)}
      ` }} />
    </div>
  );
};
...
// (unchanged logic; reads/normalises numbers & dates and composes final data)
  ...
  return (
    <div data-aos="fade-in">
      ...
      <div className="space-y-6">
        ...
        <div>
          <label htmlFor="maritalStatus" className="block text-gray-300 text-lg font-semibold mb-2">Marital Status</label>
          <select name="maritalStatus" id="maritalStatus" value={formData.maritalStatus} onChange={handleChange} className="w-full p-3 border border-gray-700 rounded-xl bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-green-500 text-lg">
        ...
        <div>
          <label htmlFor="dependents" className="block text-gray-300 text-lg font-semibold mb-2">Number of Dependents</label>
...
const OnboardingStep2 = ({ formData, setFormData, nextStep, prevStep }) => {
  const expenseCategories = [
    { name: 'housing', label: 'Housing (Rent/EMI)' }, { name: 'food', label: 'Food' }, { name: 'transportation', label: 'Transportation' },
    { name: 'utilities', label: 'Utilities' }, { name: 'entertainment', label: 'Entertainment' }, { name: 'healthcare', label: 'Healthcare' },
    { name: 'personalCare', label: 'Personal Care' }, { name: 'education', label: 'Education' }, { name: 'debtPayments', label: 'Debt Payments' },
  ...
  return (
    <div data-aos="fade-in">
      ...
      <div className="flex justify-between mt-8">
        <button onClick={prevStep} className="bg-gray-700 hover:bg-gray-600 text-white font-bold py-3 px-6 rounded-xl transition">Previous</button>
        <button onClick={nextStep} className="bg-gradient-to-r from-green-600 to-yellow-600 hover:from-green-700 hover:to-yellow-700 text-gray-900 font-bold py-3 px-6 rounded-xl transition">Next</button>
      </div>
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-lg font-semibold mb-1">Emergency Fund (₹)</label>
              <input type="text" inputMode="numeric" name="emergencyFund" value={formData.emergencyFund} onChange={handleFieldChange} className="w-full p-3 rounded-xl bg-gray-800" />
              <p className="text-xs text-gray-400 mt-1">💡 Cash in savings accounts or liquid funds for emergencies (ideally 6x monthly expenses).</p>
            </div>
            <div>
              <label className="block text-lg font-semibold mb-1">Equity Investments (₹)</label>
              <input type="text" inputMode="numeric" value={formData.investments.equity} onChange={(e) => handleNestedChange(e, 'investments', 'equity')} className="w-full p-3 rounded-xl bg-gray-800" />
              <p className="text-xs text-gray-400 mt-1">💡 Value of stocks, equity mutual funds, ELSS etc.</p>
            </div>
            <div>
              <label className="block text-lg font-semibold mb-1">Debt Investments (₹)</label>
              <input type="text" inputMode="numeric" value={formData.investments.debt} onChange={(e) => handleNestedChange(e, 'investments', 'debt')} className="w-full p-3 rounded-xl bg-gray-800" />
              <p className="text-xs text-gray-400 mt-1">💡 Value of FDs, PPF, EPF, debt funds, bonds etc.</p>
            </div>
            <div>
              <label className="block text-lg font-semibold mb-1">Real Estate (₹)</label>
              <input type="text" inputMode="numeric" value={formData.investments.realEstate} onChange={(e) => handleNestedChange(e, 'investments', 'realEstate')} className="w-full p-3 rounded-xl bg-gray-800" />
              <p className="text-xs text-gray-400 mt-1">💡 Market value of investment properties (not your primary home).</p>
            </div>
            <div>
              <label className="block text-lg font-semibold mb-1">Gold & Others (₹)</label>
              <input type="text" inputMode="numeric" value={formData.investments.gold} onChange={(e) => handleNestedChange(e, 'investments', 'gold')} className="w-full p-3 rounded-xl bg-gray-800" />
              <p className="text-xs text-gray-400 mt-1">💡 Value of physical gold, SGBs, crypto etc.</p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-lg font-semibold mb-1">High-Interest Debt (₹)</label>
              <input type="text" inputMode="numeric" value={formData.liabilities.highInterest} onChange={(e) => handleNestedChange(e, 'liabilities', 'highInterest')} className="w-full p-3 rounded-xl bg-gray-800" />
              <p className="text-xs text-gray-400 mt-1">💡 Total outstanding on credit cards, personal loans etc.</p>
            </div>
            <div>
              <label className="block text-lg font-semibold mb-1">Low-Interest Debt (₹)</label>
              <input type="text" inputMode="numeric" value={formData.liabilities.lowInterest} onChange={(e) => handleNestedChange(e, 'liabilities', 'lowInterest')} className="w-full p-3 rounded-xl bg-gray-800" />
              <p className="text-xs text-gray-400 mt-1">💡 Total outstanding on home loans, car loans etc.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div data-aos="fade-in">
      ...
      <div className="space-y-6">
        ...
        {formData.healthInsurance === 'yes' && (
          <div data-aos="fade-in">
            <label className="block text-lg font-semibold mb-1">Health Insurance Coverage (₹)</label>
            <input type="text" inputMode="numeric" name="healthInsuranceCoverage" value={formData.healthInsuranceCoverage} onChange={handleChange} className="w-full p-3 rounded-xl bg-gray-800" placeholder="e.g. 500000" />
            <p className="text-xs text-gray-400 mt-1">💡 Enter your total family floater cover amount.</p>
          </div>
        )}
        <div>
        ...
        {formData.termInsurance === 'yes' && (
          <div data-aos="fade-in">
            <label className="block text-lg font-semibold mb-1">Term Insurance Coverage (₹)</label>
            <input type="text" inputMode="numeric" name="termInsuranceCoverage" value={formData.termInsuranceCoverage} onChange={handleChange} className="w-full p-3 rounded-xl bg-gray-800" placeholder="e.g. 10000000" />
            <p className="text-xs text-gray-400 mt-1">💡 Your life cover amount. Recommended: 10-15x your annual income.</p>
          </div>
        )}
        <div>
          ...
          <select name="riskTolerance" value={formData.riskTolerance} onChange={handleChange} className="w-full p-3 border rounded-xl bg-gray-800">
            <option value="">Select one</option>
            <option value="low">Low: I prioritize safety over high returns.</option>
            <option value="medium">Medium: I'm comfortable with balanced risk for moderate growth.</option>
            <option value="high">High: I'm willing to take higher risks for potentially higher returns.</option>
          </select>
        </div>
        <div>
          ...
          <div key={index} className="bg-gray-700 p-4 rounded-xl border border-gray-600">
            <div className="flex justify-between items-center mb-3">
              ...
              {formData.customGoals.length > 1 && (
                <button type="button" onClick={() => removeGoal(index)} className="text-red-400 text-sm">Remove</button>
              )}
            </div>
            <label className="block mt-2">Goal Name</label>
            <input type="text" name="name" value={goal.name || ''} onChange={(e) => handleGoalChange(index, e)} className="w-full p-2 rounded bg-gray-800" placeholder="e.g., Retirement, Buy a Car" />
            <label className="block mt-2">Target Amount (₹)</label>
            <input type="text" inputMode="numeric" name="targetAmount" value={goal.targetAmount || ''} onChange={(e) => handleGoalChange(index, e)} className="w-full p-2 rounded bg-gray-800" />
            <label className="block mt-2">Amount Already Saved (₹)</label>
            <input type="text" inputMode="numeric" name="amountSaved" value={goal.amountSaved || ''} onChange={(e) => handleGoalChange(index, e)} className="w-full p-2 rounded bg-gray-800" />
...
const AIChat = ({ chatHistory, isGeneratingResponse, callChatAPI, financialSummary, setChatHistory }) => {
  ...
  const generateChatPrompts = () => {
    ...
    if (customGoals && customGoals.length > 0 && customGoals[0].name) {
      prompts.push(`What's the best way to invest for my "${customGoals[0].name}" goal?`);
    }
    if (prompts.length < 2) {
...
- Potential Annual Savings with this regime: **${formatIndianCurrency(Math.abs(nRT - oRT))}**
...
State the recommended tax regime and the potential savings clearly upfront.
## Detailed Comparison
Provide a clear, side-by-side comparison of the Old vs. New tax regimes.
Explain *why* one is better in this specific case.
## 💡 Smart Deduction Analysis & Missed Opportunities
Call out any 0 entries (80D, 80CCD(1B), etc.) as potential opportunities.
## Personalized Tax-Saving Strategies for Next Year
Give 2–3 actionable, personalized tips based on user context.
## Your Path Forward
    ...
    <div className="bg-gray-800 p-5 rounded-xl h-[400px]">
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie
            data={chartData}
            cx="50%" cy="50%" labelLine={false} outerRadius={120} fill="#8884d8"
...
const Dashboard = ({ financialSummary, callGroqAPIWithRetry }) => {
  ...
  useEffect(() => {
    const calculateAdvancedHealthScore = () => {
      ...
      const healthScoreValue = healthInsurance === 'yes' ? 1 : 0;
  ...
  if (!financialSummary) {
    return (
      <section className="p-8 rounded-2xl bg-gray-900 bg-opacity-80">
        <div className="flex items-center justify-center h-64"><p className="text-gray-400 text-lg">Loading your financial dashboard...</p></div>
      </section>
    );
  }
  ...
  const cGP = (g) => {
    ...
    const tA = parseFloat(g.targetAmount); const aS = parseFloat(g.amountSaved || 0);
    const p = Math.min(100, (aS / tA) * 100);
...
- Emergency Fund: ${formatIndianCurrency(financialSummary.emergencyFund)} (${(parseFloat(financialSummary.emergencyFund || 0) / (financialSummary.monthlyExpenses || 1)).toFixed(1)} months of expenses)
- High-Interest Debt: ${formatIndianCurrency(financialSummary.liabilities?.highInterest)}
- Insurance Gaps: Health: ${financialSummary.healthInsurance}, Term: ${financialSummary.termInsurance}
- Biggest Worry: "${financialSummary.financialWorry}"

**YOUR TASK:**
Provide a detailed, hyper-personalized plan in Markdown focusing on the 2–3 most impactful areas.`;
    try {
      const result = await callGroqAPIWithRetry(prompt);
...
- User's Existing Portfolio: Equity ${formatIndianCurrency(financialSummary.investments?.equity)}, Debt ${formatIndianCurrency(financialSummary.investments?.debt)}
- Monthly Surplus (for new SIPs): ${formatIndianCurrency(mS)}
- Goal: Achieve "${g.name}"
...
**YOUR TASK:**
Create a personalized, actionable, and structured investment plan (Markdown).`;
    try {
      const result = await callGroqAPIWithRetry(prompt);
              ...
              return pr ? (
                <div key={i} className="bg-gray-800 p-5 rounded-xl">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="text-right"><p className="text-sm text-gray-400">Target</p><p className="font-bold text-lg text-white">{formatIndianCurrency(g.targetAmount)}</p></div>
                  </div>
...
function App() {
  ...
  if (!GROQ_API_KEY) missing.push('REACT_APP_GROQ_API_KEY (or GROQ_API_KEY)');
...
Your tone is that of an expert, empathetic human advisor.
...
If the user asks an off-topic question, you MUST politely decline by saying: "As Zenvana, my expertise is in finance. I can't help with that, but I'm ready to answer any of your money-related questions."
Do not answer the off-topic question.

**CRITICAL INSTRUCTION: Use the following detailed user profile to make your answers deeply personal and contextual.**
---
**USER'S FINANCIAL PROFILE:**
${JSON.stringify(financialSummary, null, 2)}
---
When answering, use this context.`;
    ...
    while (retries > 0) {
      try {
        ...
        if (!response.ok) {
          const errorBody = await response.text();
          throw new Error(`Non-retriable: ${response.status}. Body: ${errorBody}`);
        }
  ...
  return (
    <div>
      ...
      {currentPage === 'onboarding' && (
        <OnboardingFlow onSubmit={saveFinancialData} initialData={financialSummary} isSubmitting={isSubmitting} />
      )}
